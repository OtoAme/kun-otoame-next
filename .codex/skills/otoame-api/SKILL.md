...
- Private chat image uploads must also re-check the recipient user's `allow_private_message` after access/type/size checks and before the real image-upload quota, moemoepoint charge, Sharp processing, or S3 upload.
