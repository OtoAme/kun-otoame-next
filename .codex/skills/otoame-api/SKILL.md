...
- Download access Phase 2 records go to `patch_resource_access`: use `user_id` for logged-in users, HTTP-only `kun-resource-access-token` for visitors, 72-hour reuse, `cost = 0`, and preview-only `obtained` / `obtainedExpiresAt`; do not add quotas or moemoepoint charging in this phase.
