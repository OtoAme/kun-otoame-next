...
- Resource create/edit/update schemas must require `official-zh`, `chinese`, `machine`, or `row` whenever `section = 'galgame'` includes `pc`, `emulator`, or `mobile`, including when `material` or `tool` is also selected; use the shared helpers in `constants/resource.ts` and return the user-visible message `请选择中文支持情况：官中、民汉、机翻、生肉`. A `galgame` resource containing only `material` or `tool` must reject any Chinese support type with `资料集或工具不允许选择中文支持类型`, while mixed game-plus-other types remain valid.
...
- Resource grant Serializable retries are capped at three attempts. Retry Prisma `P2002` / `P2034` and Prisma 7 adapter `DriverAdapterError` values only when the cause is `TransactionWriteConflict` with PostgreSQL SQLSTATE `40001`; wait 50 ms and 100 ms outside the transaction before attempts two and three. Do not retry unrelated adapter, connection, or SQL errors as grant conflicts.
