...
- Private chat message menu tests should cover keyboard access to the same bubble action menu, not only pointer context-menu or touch paths, long text-only final-line inline metadata without grid or float offsets, compact text-only centered content with bottom-right metadata, caption/reply/image bottom-right metadata, image-only translucent metadata overlays, single-message delete requiring confirmation before the delete request, and edit/delete request exceptions releasing loading state with retryable user-visible errors.
