...
- Sticker tests keep the 300 KB WebM, 100-frame estimated decode budget, 32 MiB ZIP upload, 64 MiB extracted-total, single-leading-slash archive-root normalization, traversal/absolute-path/ZIP Bomb safety, and middleware-bypass boundaries locked. Sticker admin frontend tests live in `tests/unit/admin-sticker-container.test.tsx`; cover the HeroUI master-detail layout, stable SSR Select IDs, dirty Pack selection confirmation, save availability across tabs, active-Pack physical-delete guards, card-based batch selection, selected-file visibility, import returning to the target Pack, and inline create validation. `tests/unit/kun-fetch.test.ts` keeps expected browser `AbortError` cancellation silent while preserving rejection for callers.
