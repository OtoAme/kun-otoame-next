...
- Resource grant tests must cover both Prisma `P2034` and Prisma 7 adapter `DriverAdapterError` (`TransactionWriteConflict`, SQLSTATE `40001`), the 50/100 ms transaction-external retry delays, and third-conflict exhaustion. Unit mocks do not replace the isolated real-PostgreSQL daily/weekly concurrency gate.
...
- Sticker tests keep the 300 KB WebM, 100-frame estimated decode budget, 32 MiB ZIP upload, 64 MiB extracted-total, single-leading-slash archive-root normalization, traversal/absolute-path/ZIP Bomb safety, and middleware-bypass boundaries locked. Sticker admin frontend tests live in `tests/unit/admin-sticker-container.test.tsx`; cover the HeroUI master-detail layout, sticky compact cover/status-only desktop selector, mobile status value, red/green status dots, hidden Listbox selected icons, selected-item backgrounds, stable SSR Select IDs, dirty Pack selection confirmation, save availability across tabs, invalid-Pack enable guards, enabled-Pack physical-delete guards, sticky card-based batch actions, dynamic non-cover thumbnail playback/poster replacement, selected-file visibility, import returning to the target Pack, inline import/create validation without duplicate toasts, and cleanup-warning feedback. `tests/unit/admin-sticker-pack-status-cover.test.tsx` must render the real HeroUI component to lock `content=""` plus `isDot` output instead of relying only on a Badge mock. `tests/unit/sticker-thumbnail.test.tsx` must assert that a loaded WebM remains the same mounted video node, with its playback position intact, after leaving and re-entering the viewport. `tests/unit/chat-input.test.tsx` and `tests/unit/chat-message-menu.test.tsx` keep chat picker and reply-preview WebM thumbnails animated while Pack covers remain static. `tests/unit/kun-fetch.test.ts` keeps expected browser `AbortError` cancellation silent while preserving rejection for callers.
...
- Private chat summary tests should cover `last_message_id` as the conversation-list preview source, image-only summaries, corrupted image-only summaries without valid media metadata, and deleted latest-message summaries so deleted content does not remain visible in the conversation list and dirty image rows do not look like normal image messages.
...
- Message container tests should cover first hydration keeping server-rendered notifications visible while a background current-page refresh runs, notification pagination request races so stale page responses cannot overwrite the latest requested page, pagination string errors preserving the current list with a toast, clearing read notifications requiring a confirmation before the delete request is sent, and delete string errors keeping the dialog/list open without refetching.
...
- Private chat conversation-list tests should cover first hydration keeping the server-rendered page visible while a background current-page refresh runs, background refresh syncing returned `total` into pagination, unread-chip refresh, disabled detail-link prefetch, and not clearing global unread state from a current-page-only result.
