...
- Private chat input send and image-upload request exceptions must show retryable user-visible errors and release the sending state; image-upload string responses, including rate-limit, hourly quota unavailable, insufficient moemoepoints, and expired metadata messages, must be surfaced through the existing toast branch.
