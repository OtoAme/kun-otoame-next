...
import {
  CLOUDREVE_PAN_DOMAIN,
  fetchCloudreveShareSize,
  formatSize
} from './fetchAlistSize'
...
export const createEmptyResourceLink = (section: string, role: number) => ({
...
export const ResourceLinksInput = ({
  ...
  useEffect(() => {
    if (!fields.length) {
      append(createEmptyResourceLink(section, userRole))
    }
  ...
  const handleFetchTouchGalSize = async (content: string, index: number) => {
    ...
    if (
      ...
      !content ||
      !!currentLink.size ||
      !content.includes(CLOUDREVE_PAN_DOMAIN)
    ) {
      return
    }

    toast('正在尝试从 OtoAme 官方盘获取文件大小')
    const size = await fetchCloudreveShareSize(content)
    if (size) {
      setValue(`links.${index}.size`, formatSize(size), {
        shouldValidate: true
      })
    ...
    toast.error('获取文件大小失败，请手动填写资源大小')
  }
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        ...
        <Button
          ...
          onPress={() => append(createEmptyResourceLink(section, userRole))}
        >
          添加链接
        </Button>
      </div>
