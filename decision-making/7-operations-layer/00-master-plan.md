...
模块文档索引（2026-09-08）。PM 版与实施计划使用同一组任务和实验编号；实施计划包含十二节。当前状态仍为「设计待审」，依赖的产品决定按第 9 节及各模块末节定稿。
...
| P0-3 文档准备 | 完成各模块 PM 版与十二节实施计划，统一任务、实验和依赖；01 的 D1、D2 保留明确的待定范围                                                                         | 共 20 份文档，实验编号对应；文档审阅与产品定稿分开记录                 |
...
| 任务                 | 工作与主要落点                                                                                                                               | 依赖与验收                                                                    |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| M01-1 冻结设计       | 解决队列范围、旧反馈/举报只读边界、预览布局、操作确认与权限表；核对所有旧后台路由                                                            | 模块 01 审阅完成，才开始路由迁移                                              |
...
建议目录沿用 01 草稿：`app/(site)/`、`app/(console)/console/`、`components/console/`、`styles/console.css`、`app/api/admin/inbox/`；`/console` 名称与第三个预览根布局仍在模块 01 中定稿。
...
阶段出口：审定范围内的日常审核可在收件箱完成，未迁移能力有明确入口。M01-1 按 D1 定稿来源及旧项读写范围，按 D2 定稿技术选择；验收以最终审定范围为准。
...
| M09-3 管理页迁移 | 完成下表所列旧后台页面的替代、权限与跳转，清理仅被退役页面使用的组件                                                                         | 每条旧路径有可用目标或明确归档入口，查询参数和详情 ID 不丢失              |
| M09-4 旧数据收尾 | 先盘点旧反馈/举报，按审定方案只读归档或显式迁移；历史通知链接保持可达，重复迁移可识别                                                        | 新事项只走新系统；迁移数量、归档可读性与旧写入关闭检查通过                |
| M09-5 退役发布   | 所有替代能力稳定后启用旧后台逐路由重定向，删除失去用途的处理端和入口                                                                         | 不将尚未迁移页面统一重定向到空收件箱；旧表物理删除不作为关闭旧入口的前提  |
...
| 旧路径                                                                                 | 替代归属                                                      |
| -------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| `/admin`                                                                               | 01 建立新入口；既有统计页的迁移时间在 01 定稿，最终由 09 验收 |
...
| `/admin/creator`                                                                       | 是否纳入 01 的第五来源待定；未纳入则由 09 迁移                |
| `/admin/resource`                                                                      | 06                                                            |
| `/admin/otomegame`、`/admin/comment`、`/admin/rating`、`/admin/stickers`、`/admin/log` | 09 迁移既有能力                                               |
| `/admin/user`、`/admin/user/[id]/moemoepoint`、`/admin/setting`、`/admin/email`        | 09 迁移并逐动作保持审定权限；邮件能力保持既有用途             |
...
| D1   | 第 8 份计划规定四来源、旧反馈/举报只读；01 草稿加入创作者申请并允许处理旧反馈/举报。确认是否扩展，以及如何满足「一个页面完成审核」 | M01-1                                                                                |
| D2   | 01 草稿第 12 节的 `/console`、API 单通道、危险动作确认、用户管理权限、第三根布局、实验 404、组件测试工具、统计页迁移时间           | M01-1；与已冻结规则的偏离在 01 记录理由                                              |
| D3   | 小喇叭关联条的名称、举报自动隐藏阈值与过滤细则；按当前 shadcn 模板锁定后台依赖清单                                                 | 名称/举报在 02；依赖清单在 01                                                        |
...
- [ ] 在模块 01 中逐项解决 D1、D2，定稿收件箱范围、路由与预览选择；文档审阅不代替这些决定。
- [ ] 模块 01 定稿后，依 M01-2 至 M01-5 完成实施、验证、部署与结果验收。
