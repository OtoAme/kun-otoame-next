...
- Dashboard `/dashboard/company-merges` detects legal-suffix duplicate suggestions into `company_merge_suggestion`. Detection and dismiss must not write `patch_company`. If a local DB still has `ops_case` from the operations branch, do not `prisma db push` this branch (it would drop those tables); apply only `migration/production-company-merge-suggestion-sync-2026-09-17.sql`.
