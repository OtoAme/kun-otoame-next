...
- Shoutbox rollout runs `production-shoutbox-preflight-2026-09-11.sql`, sync, postflight, then `pnpm prisma:deploy-safe` before deploying dependent code. Keep pre/postflight read-only and never substitute `prisma db push`.
...
- Company cleanup is frozen: inventory → human decisions → plan → dry → apply → cache. Only plan may call VNDB or NextMoe (`KUN_NEXTMOE_API_KEY`, optional). `pnpm exec esno scripts/probeNextmoeCompanySample.ts` is a fail-open live sample probe. Apply requires the exact SHA, locks all company state, rejects drift before writes, and emits a receipt. Cache retry verifies the complete post-state. Zero relations never imply deletion; decisions and plan must explicitly name it. Schema/tool v2 relation snapshots include `bangumiId`; do not reuse v1 inventory files.
- Dashboard `/dashboard/company-merges` detects name variants into `company_merge_suggestion`. Detection and dismiss must not write `patch_company`. Confirm-merge uses the frozen writer on the checked ids only: the smallest checked id survives, owner comes only from those companies, and each unchecked id is auto-excluded with the survivor. Skip identity is `member_key`, not `folded_key`. Pending occupancy is `company_merge_pending_key`. If a local DB still has `ops_case`, do not `prisma db push`; apply `migration/production-company-merge-suggestion-sync-2026-09-17.sql` when the table is missing, then the 2026-09-22 subset preflight/sync/postflight files in order. Do not point that SQL or tests at `otoame_prod_dump`. Restore a production dump into a separate local database before exercising merges.
