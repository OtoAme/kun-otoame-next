...
- Dashboard `/dashboard/company-merges` detects legal-suffix duplicates into `company_merge_suggestion`. Detection and dismiss must not write `patch_company`. Confirm-merge uses the frozen writer: smallest company id survives, chosen name is written onto that row, owner follows the company that originally held that name. If a local DB still has `ops_case` from the operations branch, do not `prisma db push` this branch (it would drop those tables); apply only `migration/production-company-merge-suggestion-sync-2026-09-17.sql`. Restore a production dump into a separate local database before exercising merges.
