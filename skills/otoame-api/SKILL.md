...
- Shoutbox user writes stay under `/api/shoutbox`; report writes use the existing `patch_report` carrier, while official lifecycle and moderation stay under `/api/admin/shoutbox` with `role >= 3`. Official reports may be resolved but never auto-hidden or sent through ordinary hide/remove/restore.
