...
- Shoutbox cache values carry `validUntil`; Redis TTL and anonymous `s-maxage` must never cross an official effective boundary, and non-positive dynamic TTL values are not cached. Writes that change public visibility invalidate `shoutbox:*` and the public list/banner edge cache best-effort; a below-threshold report insert does not.
