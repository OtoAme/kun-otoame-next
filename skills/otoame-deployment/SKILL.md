...
- `deploy:pull` generates the Prisma Client inside the candidate: seed the server's real `@prisma/client` package plus its declared dependencies (`@prisma/client-runtime-utils`) into the candidate `node_modules`, run `prisma generate --schema=<candidate>/prisma/schema`, and fail closed unless `node_modules/.prisma/client` appears in the candidate. Never copy the root generated client into a release candidate.
- A flattened `@prisma/client` copy without `@prisma/client-runtime-utils` fails at runtime under pnpm, and the Release artifact's pnpm standalone has no top-level `react-dom`; inject it like `deploy:build`, then preflight `require('@prisma/client')` from the candidate root before PM2 starts.
