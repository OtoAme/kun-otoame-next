...
Sticker 目录使用 `sticker_pack`、`sticker` 和 `user_sticker_pack`。后台新导入的资源把对象存储 key 写入 `storage_key` / `thumbnail_storage_key`，不把 CDN URL 当作唯一来源；`lib/s3.ts` 在读取时根据 CDN 配置派生公开 URL。动态 WebM 的 poster 必须先上传成功，Pack 上架和 Sticker 状态变更通过事务校验有效资源与同 Pack 封面。S3 上传后 DB 事务失败时必须删除本次对象，生产 schema 按 `production-private-chat-stickers-*` 再按 `production-sticker-admin-*` preflight/sync 执行。
