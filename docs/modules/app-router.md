...
新控制台由独立的 shadcn 根布局承载，不导入旧后台或 HeroUI 组件。`/dashboard` 收件箱读取投稿、资源申请、反馈和举报，统一管理员门槛；反馈与举报只读，处理链接分别沿用旧后台反馈、评论举报和评价举报页面，其写权限保持超级管理员。隔离技术验证与上线状态见模块 01 实施计划第 9、10 节。
