...
- **投稿画廊排序是显式保存的。** ready 行与待上传项在同一个 dnd-kit 网格里排序，拖拽用独立的可聚焦、键盘可达手柄。顺序跨 localforage 与数据库两个存储、无法原子提交，所以拖动只把完整的 namespaced 序列（`server:<id>` / `local:<clientAssetId>`）写进 `submission:<id>:order` draft——**该记录存在即代表未保存**，刷新后按它恢复顺序并继续禁用提交审核。保存走唯一的 `saveGalleryOrder()`：冻结快照与 generation、期间锁定其他画廊操作、返回时 generation 未变才清 draft、失败不回滚用户排好的顺序而是保留重试。未保存时点上传会先自动保存，保存不成功就中止上传。ready 行的序号允许不连续（待传卡可占中间位置），只要求唯一。
- **画廊两份草稿（items/order）的所有写入绑定 per-submission 的 DraftContext**（`{ submissionId, items, orderKeys, queue }`，队列由 `utils/serialTaskQueue.ts` 工厂生成，每次切换投稿新建 context；失败不阻断后续任务）。每个操作在事件发生时绑定一次 context，上传批次在启动时捕获并贯穿全部单文件回调；任务执行时从 context 自身的 items/orderKeys 计算、按 `ctx.submissionId` 写存储，写成功后**无条件推进 context**（卸载或切换后也推进，防止后完成的上传把前一张的旧状态写回），仅当该 context 仍是当前投稿时才同步 React/Zustand——因此滞留任务只会写自己投稿的命名空间，永远读不到新投稿的数据。拖入追加是队列任务：20 张上限与完整序列都在任务内按最新 context + 云端 gallery 重算，被拒绝的追加零写入并释放预览；拖拽排序保持事件时刻冻结的完整序列（那就是用户的手势）。同时写两个 key 的任务按崩溃可恢复方向排序：key 晋升（`local:` → `server:`）先于删除本地项，清空 order draft 永远在最后——draft 存在即未保存标记。上传成功是一个队列单元：先晋升、再删项、最后 `revokeObjectURL`；持久化失败时保留本地卡并报可重试错误，同 `clientAssetId` 重试幂等返回既有 ready 行。
