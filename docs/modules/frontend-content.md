...
- 私聊消息气泡宽度以 `ChatContainer` 内部消息滚动区的 `@container` 为口径，不能再退回 viewport 宽度。移动端/base 使用 `max-w-[70cqw]`，`md` 及以上使用 `md:max-w-[60cqw]`；带文字或引用的图片气泡保留 `md:w-[min(60cqw,32rem)]`，单图图片仍可 `w-fit` 收缩，但同样受这组最大宽度限制。
