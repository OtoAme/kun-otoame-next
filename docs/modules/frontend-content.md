...
- 游戏详情页的添加资源入口对游客保持可见，但游客点击后只显示登录/注册引导，不挂载资源填写表单；登录提示说明“普通用户满足萌萌点要求后可以发布资源”，登录用户才直接打开表单。
...
- 私聊 Sticker Pack 封面始终使用静态 WebP poster。聊天选择面板、回复预览和管理网格中的非封面动态缩略图复用 `StickerThumbnail`：接近可视区域时才首次挂载静音循环 WebM，首帧就绪后移除 poster；首次挂载后即使滚动离开可视区域也保留同一个视频节点持续播放，重新出现时不重新加载或从头开始。命中 `prefers-reduced-motion` 时仍只保留 poster，避免透明视频与静态帧叠加。
