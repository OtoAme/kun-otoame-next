...
- **画廊两份草稿（items/order）的所有写入走同一条 per-instance 串行队列**（`utils/serialTaskQueue.ts` 工厂生成，任务执行时读最新 refs；失败不阻断后续任务）。同时写两个 key 的任务按崩溃可恢复方向排序：key 晋升（`local:` → `server:`）先于删除本地项，清空 order draft 永远在最后——draft 存在即未保存标记。上传成功是一个队列单元：先晋升、再删项、最后 `revokeObjectURL`；持久化失败时保留本地卡并报可重试错误，同 `clientAssetId` 重试幂等返回既有 ready 行。
