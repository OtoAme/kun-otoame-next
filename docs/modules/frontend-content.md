...
- 资源详情表单保留“类型”标签；游戏资源分区把“中文支持”作为独立的多选上拉框，只有选择游戏类型时才显示红色必填 `*`，保留原有选项标题、说明文字和多选勾选样式。主类型下拉按“游戏类型 / 其他类型”分组，借用 HeroUI 的 `SelectSection` / `SelectItem` 默认分组标题和选项间距，不添加图标或快捷键。类型下拉通过 `popoverProps.placement = 'top'`、`shouldFlip = false` 保持从上方打开，并关闭边缘渐隐；游戏类型、工具补丁和中文支持的最大高度分别为 `360px`、`300px` 和 `256px`，刚好容纳各自的选项说明。工具补丁类型右侧显示“翻译补丁包括：民汉补丁、AI 翻译补丁、机翻补丁”。
- 资源类型校验由前端表单和 `validations/patch.ts` 共享规则共同保证：选择 PC、主机或手机游戏且未选择资料集/工具时，必须同时选择官中、民汉、机翻或生肉；选择资料集或工具后，中文支持下拉框禁用且已有中文支持值会被清除，API 也拒绝该非法组合。错误提示为“请选择中文支持情况：官中、民汉、机翻、生肉”或“资料集或工具不允许选择中文支持类型”。
