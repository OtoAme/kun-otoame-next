...
Dashboard `/dashboard/company-merges`（role ≥ 3）可手动检测法人后缀重复、驳回或合并。检测与驳回只写 `company_merge_suggestion`。合并走与冻结清理同一套写入器：主会社固定为簇里编号最小的 id，操作者选择主名与介绍来源，记录所有者跟主名原先所属会社；官网与父品牌取并集。源会社删除、旧 `/company/<id>` 404。本地库若仍有运营模块 03 的 `ops_case` 表，不要对该库跑整份 `pnpm prisma:push`（会要删掉那些表）；用 `migration/production-company-merge-suggestion-sync-2026-09-17.sql` 只建建议表。驳回不复活同一 folded key。用生产 dump 在本机演练时，应导入独立数据库，并同样只补这张建议表，不要对生产 schema 做 `db push`。冻结契约为 schema/tool v2，关系快照含 `bangumiId`；旧 inventory / plan 必须用当前 commit 重做。两种模式都会冻结动作、完整前后状态、缓存目标、工具/规范化版本和生成 commit，并写出独立 SHA sidecar。inventory、decisions、plan、sidecar、receipt、日志与数据库备份应放在仓库和所有 worktree 之外的 `0700` 持久目录，文件使用 `0600`，不得提交或跟随部署清理。
