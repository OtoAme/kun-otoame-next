...
Dashboard `/dashboard/company-merges`（role ≥ 3）可手动检测法人后缀重复并驳回建议。检测只写 `company_merge_suggestion`，不改 `patch_company`。本地库若仍有运营模块 03 的 `ops_case` 表，不要对该库跑整份 `pnpm prisma:push`（会要删掉那些表）；用 `migration/production-company-merge-suggestion-sync-2026-09-17.sql` 只建建议表。驳回不复活同一 folded key。合并确认（冻结 apply）尚未接入。冻结契约为 schema/tool v2，关系快照含 `bangumiId`；旧 inventory / plan 必须用当前 commit 重做。两种模式都会冻结动作、完整前后状态、缓存目标、工具/规范化版本和生成 commit，并写出独立 SHA sidecar。inventory、decisions、plan、sidecar、receipt、日志与数据库备份应放在仓库和所有 worktree 之外的 `0700` 持久目录，文件使用 `0600`，不得提交或跟随部署清理。
