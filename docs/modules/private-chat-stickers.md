...
- Pack 创建后默认下架；`slug` 必须是小写 snake_case，创建后不可修改，名称和描述可以编辑。
- 管理页使用 Pack 主从布局：桌面端通过 HeroUI Listbox 切换 Pack，移动端使用 HeroUI Select，右侧/下方直接编辑设置和 Sticker，不再把整个管理流程塞进大弹窗。未保存设置会固定显示保存/撤销操作，切换 Pack 前必须确认放弃。服务端渲染的 HeroUI Select 使用页面内唯一的稳定 `id`，避免 React Aria 合并自动 ID 时产生 hydration 差异。
- 上架前必须存在至少一张有效 Sticker，并且封面必须来自同一 Pack 的有效 Sticker。动态 Sticker 的封面使用其 WebP poster；导入第一张有效 Sticker 时会自动设为默认封面。
- 下架 Pack 不会删除 Sticker、对象或历史消息，但不会再出现在私聊发送面板，也不能用于新消息。禁用 Sticker 同样不影响历史消息展示。
- Sticker 支持 Pack 内全选、批量启用、批量禁用和批量删除。物理删除只用于误导入清理：Pack 必须先下架；任何被历史消息或回复引用的 Sticker 都只能禁用，不能删除；存在用户所有权记录的 Pack 也不能删除。
- 物理删除先在数据库事务中删除记录，再 best-effort 删除对象存储中的 asset、poster 和独立 Pack 封面。对象清理失败不会恢复已提交的数据库事务，接口会返回失败数量并在服务端记录具体 key，便于后续补偿。
...
- 管理页导入弹窗会列出本次选择的文件并保留服务端逐文件失败原因；导入成功后继续停留在目标 Pack 的 Sticker 标签。Pack 或 Sticker 仍处于上架状态时，永久删除按钮在前端直接禁用，API 保留最终校验。
