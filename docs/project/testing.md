...
- 会社维护：`company-identity-maintenance.test.ts` 覆盖身份盘点与 NextMoe 多家命中时的 stored/proposed 规则；`company-cleanup-frozen-*.test.ts` 覆盖 canonical inventory/decisions/plan、SHA、冻结证据、漂移零写入、事务回滚、重放与独立 cache receipt。`company-nextmoe-client.test.ts` mock `fetch`，不打真网。零关系会社不会隐式删除。
