...
- 投稿链路：`patch-submission-quota.test.ts`、`patch-submission-submit.test.ts`、`patch-submission-review.test.ts`、`patch-submission-review-route.test.ts` 和 `admin-submission-detail.test.tsx` 覆盖押金、提交/撤回审核员通知、状态机、软/硬重复外部 ID、审核并发回滚、过期审核的 `409` 与详情刷新；通知测试必须锁定「状态栅栏先成功再通知」的顺序，以及审核员查询整体失败和单个收件人投递失败都不能回滚或把已提交的状态转换报成失败；`patch-submission-publish-preview.test.ts`、`patch-submission-preview-view.test.tsx`、`patch-submission-preview-route.test.ts` 覆盖共享发布投影、正式条目外观、封面原图灯箱、分级和私有预览；`patch-submission-readonly-controls.test.tsx` 覆盖驳回投稿保留内容展示但禁用全部编辑、外部抓取与回填入口；`patch-submission-asset-cleanup.test.ts`、`patch-submission-orphan-cleanup.test.ts`、`patch-submission-upload-draft.test.ts`、`patch-submission-autosave.test.tsx`、`patch-submission-asset-route.test.ts` 和 `patch-submission-gallery-input.test.tsx` 锁定 outbox、跨刷新重试、串行保存和权限边界。
