...
- 投稿链路：`patch-submission-quota.test.ts`、`patch-submission-submit.test.ts`、`patch-submission-review.test.ts` 覆盖押金、状态机、软/硬重复外部 ID、审核并发和结算；`patch-submission-publish-preview.test.ts`、`patch-submission-preview-view.test.tsx`、`patch-submission-preview-route.test.ts` 覆盖共享发布投影、正式条目外观、封面原图灯箱、分级和私有预览；`patch-submission-asset-cleanup.test.ts`、`patch-submission-orphan-cleanup.test.ts`、`patch-submission-upload-draft.test.ts`、`patch-submission-autosave.test.tsx`、`patch-submission-asset-route.test.ts` 和 `patch-submission-gallery-input.test.tsx` 锁定 outbox、跨刷新重试、串行保存和权限边界。
...
`submission-ui.e2e.ts` 覆盖名称错误清除、自动/显式保存、服务端预览、上传失败跨刷新保留和进度重试；`patch-submission-lifecycle.e2e.ts` 从建草稿走到通知链接、审核详情和正式发布；`edit-upload-guards.e2e.ts` 覆盖 create/rewrite 上传守卫。列表页是服务端渲染的，脚本点击交互控件前必须等待客户端 hydration；模拟上传失败使用确定性的 HTTP 错误响应，不要用延迟 `route.abort()` 制造请求生命周期竞态。
