...
- 下载链接按需获取：`api/resource-access.test.ts` 覆盖资源列表脱敏、active 获取记录标记、access API 可见性/归属查询、游客 cookie、72 小时复用和 no-store 响应；`resource-download-card.test.tsx` 覆盖点击“获取下载链接”后的敏感字段展示、已获取状态按钮和错误反馈；`resource-access-links-helper.test.ts` 覆盖编辑资源前按需水合完整链接。
...
- 链接解析和提取码合并。
- 资源列表不能下发真实下载链接、提取码或解压码；下载 access API 必须校验资源归属、游戏归属和可见性，并保持 `private, no-store`。Phase 2 获取记录测试还要覆盖登录用户 / 游客 actor、72 小时复用、`cost = 0` 和列表 `obtained` 状态。
