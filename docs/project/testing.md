...
- Sticker 管理链路：`sticker-assets.test.ts` 覆盖真实 WebP、300 KB WebM、32 MiB ZIP 与 64 MiB 解压总量限制、对象 key、单前导 `/` 导出根目录归一化，以及 `..`、盘符、UNC、符号链接和 ZIP Bomb 等安全边界；`api/admin-stickers.test.ts` 覆盖 Pack 上架条件、封面保护和 S3 失败补偿；`api/admin-stickers-route.test.ts` 覆盖非管理员 API 拒绝、30 MiB ZIP 入口校验和损坏 multipart 的 `400` 降级；`middleware-matcher.test.ts` 确认大体积 Sticker 导入绕过 middleware 缓冲且相邻管理 API 仍受保护；`admin-sticker-container.test.tsx` 覆盖 Pack 主从布局、未保存设置保护、上架状态删除禁用、Sticker 批量选择、导入后保留目标 Pack、HeroUI 行内校验和服务端渲染 Select 的稳定 ID；`api/stickers.test.ts` 和 `api/conversation-service.test.ts` 覆盖下架 Pack/独立 Sticker 消息的发送校验与历史数据兼容。
