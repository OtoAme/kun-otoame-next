...
5. 填写生产域名、S3/image bed、邮件、Cloudflare、IndexNow 等生产配置。
...
animated AVIF gallery 缩略图优先使用项目依赖 `ffmpeg-static` 下载的 bundled binary，覆盖 Linux x64/arm64、macOS x64/arm64 和 Windows x64/x86 等常见平台。`ffmpeg-static` 在安装阶段下载“当前平台”的二进制，运行时 `require('ffmpeg-static')` 返回可执行文件路径。
...
系统 `ffmpeg` 只是兜底；优先路径仍然是 `ffmpeg-static`。
