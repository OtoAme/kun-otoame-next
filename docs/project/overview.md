...
- [app/api/edit/create.ts](../../app/api/edit/create.ts) 用隐藏 patch staged flow 创建游戏：短事务创建 `status = PATCH_STATUS_PUBLISHING`，事务外上传 banner 和准备外部数据，最终短事务写关系、奖励并改为 `PATCH_STATUS_VISIBLE`。
...
- create 发布不能在 Prisma interactive transaction 里做 S3、sharp 或网络请求；失败必须清理隐藏 patch 和本次已上传 banner S3 keys。
- 公开 patch 查询必须过滤 `status = PATCH_STATUS_VISIBLE`，admin/editor 查询按业务需要决定是否不过滤。
