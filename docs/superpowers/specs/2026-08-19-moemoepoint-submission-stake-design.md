...
> **已被取代（2026-08-25）。** 实现以 [用户投稿条目与云端草稿实施计划](../plans/2026-08-23-user-submission-stake.md) 为准, 当前行为见 [投稿域](../../modules/api-services.md#投稿域-patch_submission)。
>
> 本文的两条核心设计**没有被采用**：
>
> - **写入 `patch.status` 表示待审**——改为独立的 `patch_submission` 域, 审核通过前绝不写入 `patch`, 所以待审内容对公开查询天然不可见。
> - **普通驳回即罚没**——改为 `rejected`（全额返还）与 `violation`（罚没）两个终态。只有违规才扣钱。
>
> 另外文中「三个原语零调用方、不要清理」的说明已经过期：它们现在由投稿押金链路调用。
>
> 保留本文仅为记录当初的取舍与风险盘点。
