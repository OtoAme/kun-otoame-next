...
再添加七个测试：新游客授权的 expires 精确等于 now + 24h 并创建 access_kind = resource_grant 的 event；游客 count 的 where 固定含 section = galgame 和 access_kind = resource_grant；登录用户创建授权但不 count；active grant 下同一新镜像的并发 link_reveal 发生 P2034 后重读并返回 reused；active grant 下不同镜像各写一条 link_reveal；无 grant 时同资源同镜像并发由一个请求创建 resource_grant、另一个重试后 reused；无 grant 时同资源不同镜像并发由一个请求创建 resource_grant、另一个重试后创建 link_reveal。P2002 与 P2034 两种重试都要覆盖，最终只允许一条 resource_grant event。

再用真实 NextRequest/Headers 增加三项 actor 测试：设置测试 JWT_SECRET 后，首次无 Cookie 请求从 x-forwarded-for 生成不可逆且不等于原始 IP 的 ipHash；带有效 visitor cookie 的请求 ipHash 为空；登录用户 ipHash 为空。测试结束恢复原环境变量。Task 6 的 rate-limit 测试再证明只有首种 actor 会使用 visitor-ip key，且 Redis 参数和日志都不含原始 IP。
...
事务内的 quota count、grant create/update 和 access event create 必须同生同灭；刷新恢复不调用该写事务。三次仍无法完成时抛出 ResourceAccessGrantBusyError。单元测试用可控 P2002/P2034 验证重试后的四种结果；Task 7 在开发或预发布 PostgreSQL 上发真实并发请求，验证 SSI，而不是把 mock 测试当成数据库语义证明。

- [ ] **Step 6: 运行 grant 测试**

Run: pnpm test tests/unit/api/resource-access-grant.test.ts

Expected: PASS；测试应显式断言 24 小时固定到首次资源条目获取、额外镜像只写一次 link_reveal、重复查看不写、游客第 6 个游戏资源条目被拦截、登录用户无 count，以及同/不同镜像在 P2002/P2034 重试后的准确 event 分类。
...
- Produces: 单 link 响应；同 resource 的所有 preview link 共享 obtained/obtainedExpiresAt，只有有 active access event 的 link 带 revealed: true。
- Produces: 成功响应的 access.kind 明确为 resource_granted、link_revealed 或 reused；只有 resource_granted 才可能携带游客 quota。
...
再添加三项 route 测试：resource_granted 响应带 access.kind = resource_granted，且仅 visitor + galgame 可以带 quota；link_revealed/reused、登录用户和 patch 资源成功响应都不带 quota；service 抛出非预期错误时只返回通用 503/no-store，不含原始错误文本。
...
保持 Phase 2 已有的 link、resource、patch、status 与 getPatchVisibilityWhere(req) 约束。查询成功后调用 resolveResourceAccessGrant，传入 patch_id、resource_id、link_id、storage 和 section；resource_granted、link_revealed、reused 都只返回 input.linkId 对应的 content、code、password、hash，并原样映射为 access.kind。只有 result.kind === 'resource_granted' 且 result.quota 存在时才把 quota 放入成功响应；link_revealed、reused、登录用户和 patch 资源响应明确省略 quota。limited 映射为 429；ResourceAccessGrantBusyError 映射为安全的 503 + Retry-After: 1。删除 Phase 2 的 link-level existingAccess/create 分支；service 不再自行计算 expires。删除 actor.ts 的 RESOURCE_ACCESS_REUSE_MS，24 小时只由 policy.ts 的 RESOURCE_ACCESS_GRANT_MS 定义。干净基线不存在 normalizeResourceKind、hasCreatorQuota、resolveQuotaScope 或 link.resource.user_id quota 查询，不得把这些旧 Phase 3 路径移入新分支。

types/api/patch.ts 将成功语义改为最小、明确的 kind，而不是让前端从 reused 布尔值猜测是否消耗额度：
...
service 必须维持不变量：quota 存在 => access.kind === 'resource_granted'、actorType === 'visitor'、resourceKind === 'galgame'。测试同时断言反向场景全部省略 quota。
...
service 在已验证 link 后只记录 manual_reused、daily_limited 或 weekly_limited；resource_granted 与 link_revealed 已各写一条 DB event。route 在未知异常映射为通用 503 时记录 manual_failed；Task 6 接入技术限频时再记录 rate_limited。404 不记录，避免用日志形成资源枚举侧信道。access route 删除本地 resourceAccessJson 和成功分支的直接 Cookie 写入，改用 response.ts 的两个导出；actor 已建立后的 404、429、503 与成功响应都必须经过 withResourceAccessVisitorCookie，并保持 no-store。输入解析在 actor 建立前失败时仍返回 no-store 400，不创建游客身份。测试断言日志 payload 只含 operation、outcome、actorType 和可选 section，不含链接、token、IP/IP hash、actorKey、Redis key、资源 ID 或完整资源名称。
...
- Produces: PatchResourceLink.revealed?: boolean 和完整的 PatchResourceAccessRestoreResponse 类型。
...
10. 在开发或预发布 PostgreSQL 上先建立一个固定 visitor cookie，再使用尚无 grant/event 的隔离测试资源并发发送两个“同资源、同镜像”首次请求：响应应为一个 resource_granted、一个 reused；按本次验收起始时间查询 patch_resource_access，只能有一条对应镜像 event。不得让两个无 cookie 请求各自生成新 visitor identity 后冒充同 actor 并发测试。
11. 使用同一固定 visitor cookie 和另一个尚无 grant/event 的隔离测试资源，并发发送两个“同资源、不同镜像”首次请求：响应应为一个 resource_granted、一个 link_revealed；两条 event 的 expires 完全相同，且只有一条 access_kind = resource_grant。用 20 组全新的测试资源重复两类竞态，不能出现重复 event；若出现，停止发布并重新评估 partial unique index，不得把 mock P2034 测试当作通过依据。
